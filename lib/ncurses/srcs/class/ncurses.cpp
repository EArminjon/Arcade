...
void u_ncurses::printMap()
{
	...
	int y = static_cast<int>(map[0].size());
	static int color = 1;
	...
	for (size_t i = 0; i < j; ++i) {
		for (size_t c = 0; c < map[i].size(); ++c) {
			if (this->map[i][c] != ' ') {
				...
				mvprintw(static_cast<int>(LINES / 4 - (map.size() / 2) + i),
						 static_cast<int>(COLS / 2 - (y / 2) + c), "#");
	...
	mvprintw(LINES / 3 + 2,
			 static_cast<int>(COLS / 2 - this->playerName.size() / 2),
...
void u_ncurses::printSnake()
{
	...
	auto y = static_cast<int>(snake[0].size() + 1);
	static int color = 10;

	for (size_t i = 0; i < j; ++i) {
		attron(COLOR_PAIR(color));
		mvprintw(static_cast<int>(LINES - snake.size() - 1 + i), 1,
				 this->snake[i].c_str());
		mvprintw(static_cast<int>(LINES - snake.size() - 1 + i), COLS - y,
				 this->r_snake[i].c_str());
		attroff(COLOR_PAIR(color));
		++color;
		if (color == 15)
			color = 10;
...
void u_ncurses::printCentipede()
{
	...
	auto z = static_cast<int>(centipede[2].size() + 1);
	static int color = 10;

	for (size_t i = 0; i < j; ++i) {
		attron(COLOR_PAIR(color));
		mvprintw(static_cast<int>(LINES - centipede.size() - 1 + i), COLS / 2 - z / 2,
...
void u_ncurses::printPacman()
{
	...
	auto y = static_cast<int>(fantoms[0].size() + 1);
	auto z = static_cast<int>(pacman[0].size() + 1);
	static int color = 9;

	for (size_t i = 0; i < j; ++i) {
		attron(COLOR_PAIR(color));
		mvprintw(static_cast<int>(LINES - fantoms.size() - 1 + i), COLS / 2 - y / 2,
				 (this->fantoms[i].c_str()));
		attroff(COLOR_PAIR(color));
		attron(COLOR_PAIR(14));
		mvprintw(static_cast<int>(LINES - pacman.size() - 1 + i), COLS / 2 - z / 2,
				 (this->pacman[i].c_str()));
							...
							std::map<std::string, void *> libs)
{
	...
	for (auto game = games.begin(); game != games.end(); ++game, ++index) {
		...
		mvprintw(LINES / 2 - 2 + index * 2,
				 static_cast<int>(COLS / 3 - game_name.size() / 2),
				 game_name.c_str());
		nb += 2;
	}
	index = 0;
	for (auto lib = libs.begin(); lib != libs.end(); ++lib, ++index) {
		...
		std::transform(lib_name.begin(), lib_name.end(), lib_name.begin(),
					   ::toupper);
		mvprintw(LINES / 2 - 2 + index * 2,
				 static_cast<int>(COLS - COLS / 3 - lib_name.size() / 2), lib_name.c_str());
		nb += 2;
	}
	mvprintw(static_cast<int>(LINES / 2 - 2 + libs.size() * 2), COLS - (COLS / 3) - 2, "EXIT");
	if (this->choose < games.size())
		nb = static_cast<int>(COLS / 3 - my_substr(gameIndex->first, GAMES_LIB_PATH,
												   GAMES_LIB_EXT).size() / 2);
	else if (this->choose < games.size() + libs.size())
		nb = static_cast<int>(COLS - COLS / 3 - my_substr(libIndex->first, GRAPHICS_LIB_PATH,
														  GRAPHICS_LIB_EXT).size() / 2);
	else
		nb = COLS - COLS / 3 - 2;
	if (this->choose < games.size()) {
		selectSnake(static_cast<int>(my_substr(gameIndex->first, GAMES_LIB_PATH,
											   GAMES_LIB_EXT).size()), nb,
					static_cast<int>(-1 + this->choose * 2));
		...
		selectSnake(6, nb,
					static_cast<int>(-1 + (this->choose - games.size()) * 2));
}
